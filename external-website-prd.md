...
my account页面关于金额不要显示美元 显示人民币

后台 首页/关于我们 图文配置 前端能生效  **系统固定文案** (如导航栏、按钮文字): 没有配置的文字部分 中英文走翻译文件

需要确认订单页 checkout 再改（因为现在前端点击购物车页面的确认订单按钮是报错的 ）需要逻辑闭环 

订单管理逻辑，前后端联调 前端确认订单后（提示已经收到您的订单）然后 后台能看到
