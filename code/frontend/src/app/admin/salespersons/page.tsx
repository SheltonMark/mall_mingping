...
export default function SalespersonsPage() {
  ...
  const handleDelete = async (id: string, salesperson: Salesperson) => {
    // 检查是否有关联客户或订单
    const customerCount = salesperson._count?.customers || 0;
    const orderCount = salesperson._count?.orders || 0;

    if (customerCount > 0 || orderCount > 0) {
      if (!confirm(
        `该业务员有 ${customerCount} 个客户和 ${orderCount} 个订单。\n删除后这些数据将不再关联业务员。\n确定要删除吗？`
      )) {
        return;
      }
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await salespersonApi.update(editingId, submitData);
        alert('更新成功');
      } else {
        await salespersonApi.create(submitData);
        alert('创建成功');
      }
                ...
                {salespersons.map((salesperson) => (
                  <tr key={salesperson.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                      ...
                      <button
                        onClick={() => handleDelete(salesperson.id, salesperson)}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  {editingId && (
                    <p className="text-xs text-gray-500 mt-1">工号不可修改</p>
                  )}
                </div>
