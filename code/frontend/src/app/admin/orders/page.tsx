              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {filteredOrders.map((order) => {
                  ...
                  <tr key={order.id} className={`hover:bg-gray-50 ${selectedOrderIds.has(order.id) ? 'bg-blue-50' : ''}`}>
                    ...
                    <td className="px-4 py-4">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium rounded-full ${statusConfig.bgColor} ${statusConfig.color}`}>
                        <StatusIcon size={12} />
                        {statusConfig.label}
                      </span>
                    </td>
                    <td className="px-4 py-4">
                      <div className="text-sm font-medium text-gray-900">{order.salesperson?.chineseName || '-'}</div>
                      <div className="text-xs text-gray-500">{order.salesperson?.accountId}</div>
                    </td>
                    <td className="px-4 py-4">
                      <div className="text-sm text-gray-900" title={order.customer?.name || order.erpCustomer?.name || '-'}>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {order.status === 'PENDING' && (
                          <button
                            ...
                            className="flex items-center gap-1 px-2 py-1 text-xs text-yellow-600 hover:bg-yellow-50 rounded transition-colors"
                            title="审核订单"
                          >
                            <CheckCircle size={14} />
                            审核
